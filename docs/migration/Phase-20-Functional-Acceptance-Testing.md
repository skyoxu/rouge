...
> **核心目标**：逐功能对标验证 旧项目 与 rouge 的特性一致性，建立功能完整性与兼容性验收基线。
> **工作量**：5-7 人天
...
| 挑战 | 原因 | Godot 解决方案 |
|-----|-----|-----------:|
| 功能对标验证 | 旧前端游戏引擎/旧前端框架 迁移到 Godot 可能遗漏功能 | 逐功能对标清单 + 自动化测试用例 |
...
| 功能模块 | 旧项目 实现 | rouge 对标 | 验收标准 |
|---------|-------------|---------------|---------|
| **主菜单** | 旧前端框架 菜单组件 + EventBus | Godot Control 场景 | 菜单导航功能一致 |
| **游戏场景** | 旧前端游戏引擎 Scene Tree | Godot Scene Tree | 场景初始化、对象生成、销毁一致 |
| **角色控制** | 旧前端游戏引擎 精灵 + 输入系统 | Godot CharacterBody2D | 移动、跳跃、攻击行为一致 |
| **敌人 AI** | 旧前端游戏引擎 AI 逻辑 | Godot State Machine + FSM | 敌人行为逻辑一致 |
| **物理系统** | 旧前端游戏引擎 Physics | Godot Physics2D | 碰撞、重力、速度计算一致 |
...
| **数据导入** | - | 迁移脚本 | 旧项目 数据可导入 rouge |

#### 维度 D: 性能基线对标（Performance Baseline）

| 性能指标 | 旧项目 基线 | rouge 目标 | 验收标准 |
|--------|-------------|--------------|---------|
...
│   │
│   └── e2e/
│       ├── smoke-tests.旧 E2E 工具.ts          冒烟测试脚本
│       └── user-flow-tests.旧 E2E 工具.ts      用户流程测试
│
├── docs/
...
namespace Game.Tests.Acceptance
{
    ...
    public class FeatureMappingTests
    {
        ...
        {
            // Arrange
            var vitegameExportFile = "/path/to/旧项目-export.json";
...
- 快速验证游戏基本可玩性
- 检查菜单导航、场景加载、基本交互
- 使用 旧 E2E 工具 Godot 支持的 E2E 测试

**代码示例**：

```typescript
// tests/e2e/smoke-tests.旧 E2E 工具.ts
...
class AcceptanceTestRunner:
    ...
    def _run_smoke_tests(self):
        ...
        cmd = [
            "npx", "旧 E2E 工具", "test",
            str(self.project_root / "tests/e2e/smoke-tests.旧 E2E 工具.ts"),
            "--reporter=json",
...
- [ ] smoke-tests.旧 E2E 工具.ts（300+ 行）：[OK] 冒烟测试脚本
...
- [OK] `tests/e2e/smoke-tests.旧 E2E 工具.ts`（300+ 行）
