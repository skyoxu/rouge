...
**旧桌面壳 + 旧前端游戏引擎 3 性能基线**：
- 启动时间：冷启动 <2.0s（包含 旧桌面壳 初始化 + 旧脚本运行时 runtime）
- 帧时间：60 FPS @ 1080p（旧前端游戏引擎 WebGL 渲染，Chrome 引擎优化）
- 内存占用：峰值 ~200MB（包含 V8 堆 + 旧浏览器运行时 渲染进程）
- CPU 占用：空闲 <25%（主线程 + 渲染线程 + Web Worker）
- 资源加载：首屏资源 <500KB，懒加载策略

**已知性能瓶颈**：
- 旧桌面壳 容器开销：~50-80MB 基础内存（旧浏览器运行时 + 旧脚本运行时）
- 旧前端框架 虚拟 DOM：复杂 UI 重绘可能触发卡顿（>16.67ms）
- 旧前端游戏引擎 场景切换：首次场景加载 ~800ms（纹理解压 + WebGL 初始化）
