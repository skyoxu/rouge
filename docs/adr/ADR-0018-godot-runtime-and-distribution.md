...
- Status: Accepted
- Context: Migration Phase-2（docs/migration/Phase-2-ADR-Updates.md），对齐 CH01/CH03 基线；Windows-only 交付。原项目使用 旧桌面壳 + 旧前端框架 + 旧前端游戏引擎 + TypeScript 技术栈，本仓库迁移为 Godot 4.5 + C#/.NET 8 模板，需要统一运行时、发布与测试/门禁口径，避免契约与流程分叉。
- Decision:
...
- Consequences:
  - 旧桌面壳/旧浏览器运行时 运行时退役；安全基线从“旧桌面壳 安全”切换为“Godot 安全”（见 ADR-0019）。
