# ADR-0031: Core 线程模型（Single-Thread Core + 明确跨线程边界）
...
- 领域对象（例如玩家、局内 RunState、地图状态、经济/背包状态等）必须在创建它们的线程内使用与变更。
- 通过运行时断言/（可选）线程访问守卫强制执行（运行时 fail-fast）。
...
- 禁止订阅者做：
  - 直接修改 `PlayerState` / `RunState` / `MapState` 等 Core 状态
