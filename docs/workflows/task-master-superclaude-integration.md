...
> 说明：在本仓库（rouge）中，Taskmaster 任务存放于 `.taskmaster/tasks/*.json`（如 tasks_back.json、tasks_gameplay.json、tasks_longterm.json）；下文以 tasks.json 字段示例表示单条任务记录的典型结构，具体字段命名以 `.taskmaster/tasks/*.json` 为准。
...
```json
{
  ...
  "adrRefs": ["ADR-0002"],
  "archRefs": ["CH01", "CH05"],
  "overlay": "docs/architecture/overlays/PRD-rouge-manager/08/..."
}
```
...
| **Orchestration** | 需要串联多个 MCP 工具 | 优化工具调度效率 | Context7 → Serena → 旧 E2E 工具 流水线 |
...
3. **旧 E2E 工具 MCP** (可选，主要面向 Web/旧桌面壳 子项目)
   - 用途: E2E 回归测试自动化（仅当项目存在 HTML5/Web/旧桌面壳 前端时适用），默认 Godot+C# 模板的 E2E 由 GdUnit4/headless Godot 承担。
- 场景: Web UI 测试、旧桌面壳 包装应用的回归验证（默认不开启）。
   - 配置: 如需启用，需在项目层单独配置 旧 E2E 工具 与 MCP 集成，不属于模板必备能力。
...
| 需要串联 Context7/Serena/旧 E2E 工具 | Orchestration | `/sc:analyze --mode orchestration --task 1.1 --mcp context7,serena` |
...
/sc:analyze --mode orchestration --task 1.1 --mcp context7,serena,旧 E2E 工具
```
...
```
 ...
 命名规范: 100% 符合 PascalCase/camelCase 约定
  建议: GuildCreationTests.cs:45 测试名可更明确
```
...
**3.17 旧 E2E 工具 MCP 集成(可选)**

** 重要提示**：本项目为 **Windows Desktop 游戏**（ADR-0011），默认导出为原生 `.exe`。
旧 E2E 工具 仅在以下情况有用：
- 项目有 **HTML5 Web 版本**计划
- 需要测试 **旧桌面壳 包装**的桌面版
...
使用 旧 E2E 工具 MCP 进行 E2E 回归测试(适用于 Godot 导出的 HTML5 版本或 旧桌面壳 包装):

```bash
# 在 Claude Code 对话中使用 旧 E2E 工具
@旧 E2E 工具 test
# 运行所有 E2E 测试

@旧 E2E 工具 test tests/e2e/guild-creation.spec.ts
# 运行特定测试文件
...
**配置 旧 E2E 工具 MCP**:
```bash
# 在 Claude Code 中启用 旧 E2E 工具 MCP
# 详见 SuperClaude v4 安装清单(Phase 6)
claude mcp add 旧 E2E 工具
```
...
```bash
# 主分支保持在 main
git worktree add ../rouge-task-1.1 -b feature/task-1.1
git worktree add ../rouge-task-1.2 -b feature/task-1.2

# 在不同终端/IDE 实例中分别处理
# Terminal 1: cd ../rouge-task-1.1 && code .
# Terminal 2: cd ../rouge-task-1.2 && code .
```
...
```json
{
  "superclaude": {
    ...
    "mcp": {
      "enabled": ["serena", "context7", "旧 E2E 工具"],
      "default": ["serena"]
    }
  }
}
...
# 5. E2E 回归测试
@旧 E2E 工具 test tests/e2e/smoke.spec.ts
...
- [ ] 旧 E2E 工具 MCP 连接正常（如已安装）：`@旧 E2E 工具 test --help`
...
- 在本仓库（rouge）中，**主线工具链** 是：`.taskmaster/tasks/*.json` + Python 脚本 + dotnet/xUnit + GdUnit4，用于驱动 PRD/Base/ADR 约束下的 Godot+C# 游戏开发。
- 文档中出现的 Node / npm / 旧 E2E 工具 MCP 等内容，默认视为 **仅在存在 Web/HTML5/旧桌面壳 子项目时启用的可选能力**，不是本仓库的硬依赖。
- 如果当前项目只构建原生 Windows Godot 游戏，可以暂时忽略所有 Node/Vitest/旧 E2E 工具 相关命令与脚本，不影响 T2 场景和核心回合循环的实现与验证。
- 当你未来引入 Web/旧桌面壳 前端、需要前端专用 E2E/可用性测试时，可以参考文档中的 Node/Vitest/旧 E2E 工具 段落，将其视为在本仓库主线之上的“额外层”，并保持 ADR/任务回链与现有 Godot+C# 工具链一致。
